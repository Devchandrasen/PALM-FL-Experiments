...
def mean_by_operating_point(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for row in rows:
        groups.setdefault((row.get("dataset", ""), row.get("mode", ""), row.get("scheduler", "")), []).append(row)
    ...
    for (dataset, mode, scheduler), members in sorted(
        groups.items(),
        key=lambda item: (dataset_order.get(item[0][0], 99), mode_order.get(item[0][1], 99), item[0][2]),
    ):
        item: dict[str, str] = {"dataset": dataset, "mode": mode, "scheduler": scheduler, "n": str(len(members))}
...
def plot_accuracy_energy(rows: list[dict[str, str]], out: Path) -> None:
    rows = mean_by_operating_point(rows)
...
def plot_comm_privacy(rows: list[dict[str, str]], out: Path) -> None:
    rows = [r for r in mean_by_operating_point(rows) if r.get("mode") in {"Local-only", "Stats-only", "PALM-transfer"}]
